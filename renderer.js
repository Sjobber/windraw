document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById("winners").addEventListener("click", function () {
        ...
        entries.forEach(entry => {
            const regex = /^([\w'’]+)\s(\w).*?(\d+(\.\d+)?)/; // Match first word (including special characters) and first letter after space
            const match = entry.trim().match(regex);

            if (match) {
                const name = match[1] + " " + match[2]; // Username is first word (including special characters) and first letter after space
                const number = parseFloat(match[3]); // Extract number (supporting decimals) and treat "." and "," as the same character
